...
class OpenAIServingModels:
    ...
    async def init_static_control_vectors(self):
        ...
        for cv in self.static_control_vectors:
            cv_request = LoadControlVectorRequest(
                cv_path=cv.path, cv_name=cv.name, cv_scale=cv.scale_factor
            )
    ...
    async def load_control_vector(
        ...
        logger.info(
            "Loaded new control vector: name '%s', path '%s'", cv_name, cv_path
        )
    ...
    async def _check_load_control_vector_request(
        ...
        # Check if the lora adapter with the given name already exists
        if any(
            cv_request.control_vector_name == request.cv_name
            for cv_request in self.cv_requests
        ):
            return create_error_response(
                message=f"The control vector '{request.cv_name}' has already been "
                "loaded.",
                err_type="InvalidUserInput",
                status_code=HTTPStatus.BAD_REQUEST,
            )
    ...
    async def _check_unload_control_vector_request(
        ...
        if not request.cv_name and not request.cv_int_id:
            return create_error_response(
                message="either 'cv_name' and 'cv_int_id' needs to be provided.",
                err_type="InvalidUserInput",
                status_code=HTTPStatus.BAD_REQUEST,
            )
        ...
        if not any(
            ...
            return create_error_response(
                message=f"The control vector '{request.cv_name}' cannot be found.",
                err_type="NotFoundError",
                status_code=HTTPStatus.NOT_FOUND,
            )
